    ...
    <v-card class="pt-3">
      <v-card-text>
        ...
        <v-form @submit.prevent="handleFormSubmit">
          <v-text-field type="text" placeholder="Link Title" v-model="data.title" />
          <v-select
            label="Select Token"
            :items="config.tokens"
            item-text="symbol"
            ...
            v-model="data.token"
          />
          <v-text-field type="number" placeholder="Enter Amount" v-model="data.amount" />
          <v-textarea placeholder="Link Description" v-model="data.description" />
...
export default defineComponent({
  setup() {
    ...
    return { authStore, config, show, util, toggleCreateSupportLinkModal, data, handleFormSubmit, loaders }
  },
})
</script>
